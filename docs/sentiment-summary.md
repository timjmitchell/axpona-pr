...
# Force fresh extraction (ignore checkpoints)
python scripts/extract_sentiment.py --no-resume
```

**Resume on crash:** The script checkpoints results after each source completes. If killed mid-run, re-running the same command picks up where it left off — only un-extracted sources hit the API. Checkpoints are cleared after successful completion.
